...
import {RESET_TRIP_CREATED, RESET_TRIPPASSENGER_CREATED} from "../../redux/types/TripTypes";
const {Option} = Select;
...
function AddTripPassenger(props) {
	...
	console.log(" tripCreated in ra coi", tripCreated);
...
function AddPoint(props) {
	...
	const onFinish = (values) => {
		if (!tripPassengerCreated || !tripPassengerCreated.id) {
			message.error("Chưa có thông tin chuyến xe phụ trách, vui lòng quay lại bước trước");
			return;
		}
		console.log("AddPoint - onFinish values:", values); // <-- thêm dòng này
		...
		console.log("AddPoint - createdPoint payload:", createdPoint); // <-- thêm dòng này
		dispatch(createTimePointTripAction(createdPoint));
		// ✅ Reset trip state SAU KHI HOÀN TẤT
		dispatch({type: RESET_TRIP_CREATED});
		dispatch({type: RESET_TRIPPASSENGER_CREATED});
	};
