...
export default function Details(props) {
	...
	console.log("Danh sách comments", listCommentPassenger);
	...
	const renderComment = () => {
		let filteredComments = listCommentPassenger;

		if (selectedRate !== null) {
			filteredComments = listCommentPassenger?.filter((item) => {
				const rates = item.userComment.userRate?.filter((rate) => rate.passengerId === item.passengerId);

				if (!rates || rates.length === 0) return false;

				const avgRate = _.meanBy(rates, "numberRate");
				return Math.round(avgRate) === selectedRate;
			});
		}

		if (!filteredComments || filteredComments.length === 0) {
			return <p style={{marginTop: 16}}>Không có đánh giá</p>;
		}

		return filteredComments?.map((item, index) => {
			const rates = item.userComment.userRate?.filter((rate) => rate.passengerId === item.passengerId);
			const avgRate = rates?.length ? _.meanBy(rates, "numberRate") : 0;
				...
				<Comment
					...
					author={
						<a>
							...
							<div>{avgRate === 0 ? <p>Không có đánh giá</p> : <Rate disabled defaultValue={avgRate} style={{fontSize: 10}} />}</div>
						</a>
					}
					...
					{isLoadingSpin ? (
						...
						<>
							...
							<div className="ant-row filter-rate">
								<button type="button" className={`ant-btn button-filter ${selectedRate === null ? "active" : ""}`} onClick={() => setSelectedRate(null)}>
									<span>Tất cả ({listCommentPassenger?.length || 0})</span>
								</button>

								{[5, 4, 3, 2, 1].map((rate) => (
									<button key={rate} type="button" className={`ant-btn button-filter ${selectedRate === rate ? "active" : ""}`} onClick={() => setSelectedRate(rate)}>
										<span>{rate}</span>
