...
function StepBookingSeat(props) {
	...
	const [tripSeats, setTripSeats] = React.useState([]);
	...
	React.useEffect(() => {
		if (tripPassenger?.id) {
			fetch(`${DOMAIN}tripseat/${tripPassenger.id}`)
				...
				.catch((error) => {
					console.error("Lấy dữ liệu ghế thất bại:", error);
				});
		}
	}, [tripPassenger?.id]);
	...
	const renderSeat = (floor) => {
		const seatsOnFloor = tripSeats.filter((item) => item.floor === floor);

		const seatRows = [];
		for (let i = 0; i < seatsOnFloor.length; i += 2) {
			const leftSeat = seatsOnFloor[i];
			const rightSeat = seatsOnFloor[i + 1]; // Có thể là undefined nếu lẻ

			seatRows.push(
				<div className="flex justify-between items-center mb-2" key={`row-${i}`}>
					{/* Ghế bên trái */}
					{renderSingleSeat(leftSeat, `left-${i}`)}

					{/* Khoảng cách giữa 2 bên, hoặc spacer */}
					{/* <div className="w-2" /> */}

					{/* Ghế bên phải */}
					{rightSeat ? renderSingleSeat(rightSeat, `right-${i + 1}`) : <div className="w-12 h-12" />}
				</div>
			);
		}
	...
	const renderSingleSeat = (item, key) => {
		if (!item) return <div className="w-12 h-12" key={key}></div>; // Placeholder nếu không có ghế

		const isBooked = item.status === "đã đặt";
		const isSelecting = listSeatSelected.some((seatSelect) => seatSelect.id === item.id);

		const seatClasses = ["border-none p-0 m-0", isBooked ? "opacity-50 cursor-not-allowed" : "cursor-pointer", isSelecting ? "ring-2 ring-blue-500" : ""].join(" ");

		const typeGhe = item.type === "seat" ? "Ghế thường" : "Giường nằm";

		return (
			<Tooltip key={key} title={`Ghế: ${item.name}, Giá: ${item.price.toLocaleString()} VNĐ, Loại: ${typeGhe}`} placement="top">
				<button className={seatClasses} onClick={() => dispatch(bookingSeatAction(item))} disabled={isBooked}>
					<div className="w-12 h-12 flex items-center justify-center bg-gray-200 rounded">
						<Seat arrClass={[isBooked ? "gheDaChon" : "", isSelecting ? "GheDangChon" : ""]} />
					</div>
				</button>
			</Tooltip>
		);
			...
			{isLoadingSpin ? (
				...
				<>
					<div class="TrustMessage__Container-sc-8xur6b-0 deVKGv trust-message-container undefined" style={{color: "rgb(14, 99, 193)"}}>
						<p class="base__Body02-sc-1tvbuqk-14 VqdXU trust-message-content">XeNhanh cam kết giữ đúng vị trí bạn đã chọn.</p>
					</div>
	...
	return (
		<div className="info-trip">
			<div className="TrustMessage__Container-sc-8xur6b-0 deVKGv trust-message-container undefined">
				<p className="base__Body02-sc-1tvbuqk-14 VqdXU trust-message-content">Chỉ còn 1 bước nữa! XeNhanh cam kết chỉ sử dụng thông tin trong việc ghi nhận vé.</p>
			</div>
			...
			<div className="noteInfo">
				...
				<div className="info-note-content">
					...
					<p className="base__Body02-sc-1tvbuqk-14 VqdXU color--dark">Để đảm bảo đúng thông tin khi di chuyển, vui lòng nhập số CMND/CCCD chính xác theo quy định hiện hành.</p>
				</div>
			</div>
			<div class="footer">
				<div class="footer-left"></div>
