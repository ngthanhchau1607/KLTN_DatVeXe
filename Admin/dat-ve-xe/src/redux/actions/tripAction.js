		...
		try {
			...
			if (result.status == 200) {
				...
				dispatch({
					...
					tripPassengerCreated: result.data.tripPassenger,
				});
				next();
			}
		} catch (error) {
...
export const getTripPassengersByTripsAction = (tripIds) => {
	return async (dispatch) => {
		try {
			const allTripPassengers = [];
			for (let tripId of tripIds) {
				const res = await tripService.getTripPassenger(tripId);
				if (res.status === 200) {
					allTripPassengers.push(
						...res.data.map((item) => ({
							...item,
							openDetail: false,
							openBooking: false,
